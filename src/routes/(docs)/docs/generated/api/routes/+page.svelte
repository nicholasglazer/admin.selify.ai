<script>
  import {Clock, RefreshCw, AlertCircle, Route, ChevronRight, Search, X, Globe} from '@lucide/svelte';
  ...
  // Extract route prefixes from HTML for TOC
  let routePrefixes = $derived.by(() => {
    if (!data.html) return [];
  ...
  let filteredRoutes = $derived(
    searchQuery
      ? routePrefixes.filter(t => t.name.toLowerCase().includes(searchQuery.toLowerCase()))
      : routePrefixes
  );
  ...
  {:else}
    ...
    <aside class="toc-sidebar">
      <div class="toc-header">
        <Globe size={16} />
        <span class="toc-title">Route Prefixes</span>
        <Badge variant="secondary" size="sm">{routePrefixes.length}</Badge>
      </div>
      ...
      <nav class="toc-list">
        {#each filteredRoutes as route (route.id)}
          <button
            class="toc-item"
            class:active={activeSection === route.id}
        ...
        {#if filteredRoutes.length === 0}
          <p class="no-results">No routes match "{searchQuery}"</p>
        {/if}
      </nav>
    </aside>
...
<style lang="postcss">
  ...
  .toc-item.active {
    @apply bg-base0C/15 text-base0C;
  }
  ...
  .doc-content :global(h4) {
    @apply text-base font-medium text-base05 mt-4 mb-2;
  }
  ...
  .doc-content :global(blockquote) {
    @apply border-l-4 border-base0C pl-4 text-base04 my-4;
  ...
  /* HTTP Method Badges */
  .doc-content :global(code:first-child) {
    @apply font-semibold;
  }
